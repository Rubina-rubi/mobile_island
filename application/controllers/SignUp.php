...
class SignUp extends CI_Controller {
	...
	public function index()
	{
			...
			$this->form_validation->set_rules('name', 'Name', 'required|regex_match[/^[A-Z a-z ]+$/]');
			...
			$this->form_validation->set_rules('mobile', 'Mobile', 'required|is_numeric|min_length[11]');
		...
		$this->load->view('inc/navbar',$o_data);
				...
				$edata['ulink']='http://localhost/ams/verify/pass/'.$last_id.'';
				$edata['uname']=$data['name'];
				$edata['uemail']=$data['email'];
				
				$this->load->view('emailsent',$edata);
					
					$msg='<div class="alert alert-success">Congratulation Your Account Created. Please Check Email to Verify </div>';
